...
这是参考图玩法的可玩纵向切片：玩家从港口出发，在更大的 9×9 航区中使用右下角常驻雷达寻找多个鱼群，驾驶船沿海格航行并破冰。进入鱼群发现范围后，鱼群模型、稀有度与价值会显示出来，玩家按 E 获取；每个鱼群占用一个货舱格。货舱装满或航区没有剩余鱼群时返港出售，再用金币升级货舱并开始下一轮。
...
- 顶部目标轨道、金币/燃料 HUD、0/7 起步的货舱面板、珍稀收藏和 9×9 实时小地图共同提示当前状态。
